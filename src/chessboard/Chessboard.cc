...
Chessboard::Chessboard(cv::Size boardSize, cv::Mat& image, float squareSize, float markerSize) : mBoardSize(boardSize), mCornersFound(false) {
    if (image.channels() == 1) {
    ...
    dictionary_ = cv::aruco::getPredefinedDictionary(cv::aruco::DICT_4X4_250);  // TODO extend dictionary type
    board_ = cv::aruco::CharucoBoard::create(squaresX, squaresY, squareLength, markerLength, dictionary_);
}

void Chessboard::findCorners(bool useOpenCV, bool useCharuco) {
    int flags = cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_NORMALIZE_IMAGE + cv::CALIB_CB_FILTER_QUADS + cv::CALIB_CB_FAST_CHECK;
    mCornersFound = findChessboardCorners(mImage, mBoardSize, mCorners, ids_, flags, useOpenCV, useCharuco);

    if (mCornersFound) {
        if (useCharuco) {
            cv::aruco::drawDetectedCornersCharuco(mSketch, mCorners, ids_, cv::Scalar(255, 0, 0));
        } else {
...
bool Chessboard::findChessboardCorners(const cv::Mat& image, const cv::Size& patternSize, std::vector<cv::Point2f>& corners, std::vector<int>& ids, int flags,
                                       bool useOpenCV, bool useCharuco) {
    if (useCharuco) {
        cv::Mat gray;
        if (image.channels() == 1) {
            gray = image.clone();
        ...
        if (marker_ids.size() == 0) return false;

        cv::aruco::interpolateCornersCharuco(marker_corners, marker_ids, gray, board_, corners, ids);

        return corners.size() == mBoardSize.width * mBoardSize.height;
...
bool Chessboard::findChessboardCornersImproved(const cv::Mat& image, const cv::Size& patternSize, std::vector<cv::Point2f>& corners, int flags) {
    ...
    for (int k = 0; k < 6; ++k) {
        for (int dilations = minDilations; dilations <= maxDilations; ++dilations) {
            ...
            if (dilations >= 1) cv::dilate(thresh_img, thresh_img, kernel1);
            if (dilations >= 2) cv::dilate(thresh_img, thresh_img, kernel2);
            if (dilations >= 3) cv::dilate(thresh_img, thresh_img, kernel1);
            if (dilations >= 4) cv::dilate(thresh_img, thresh_img, kernel2);
            if (dilations >= 5) cv::dilate(thresh_img, thresh_img, kernel1);
            if (dilations >= 6) cv::dilate(thresh_img, thresh_img, kernel2);
...
bool Chessboard::checkQuadGroup(std::vector<ChessboardQuadPtr>& quads, std::vector<ChessboardCornerPtr>& corners, cv::Size patternSize) {
    ...
    if ((int)corners.size() != patternSize.width * patternSize.height || linkedBorderCorners < (patternSize.width * 2 + patternSize.height * 2 - 2) * 0.75f) {
        return false;
    }
...
void Chessboard::getQuadrangleHypotheses(const std::vector<std::vector<cv::Point>>& contours, std::vector<std::pair<float, int>>& quads, int classId) const {
...
bool less_pred(const std::pair<float, int>& p1, const std::pair<float, int>& p2) { return p1.first < p2.first; }
