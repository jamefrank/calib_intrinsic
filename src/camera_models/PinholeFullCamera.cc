...
PinholeFullCamera::Parameters::Parameters(const std::string& cameraName,  //
                                          int w, int h, double k1, double k2, double k3, double k4, double k5, double k6, double p1, double p2, double fx,
...
bool PinholeFullCamera::Parameters::readFromYamlFile(const std::string& filename) {
...
void PinholeFullCamera::Parameters::writeToYamlFile(const std::string& filename) const {
    ...
    fs << "{"
       << "k1" << m_k1 << "k2" << m_k2 << "k3" << m_k3 << "k4" << m_k4 << "k5" << m_k5 << "k6" << m_k6 << "p1" << m_p1 << "p2" << m_p2 << "}";
...
PinholeFullCamera::Parameters& PinholeFullCamera::Parameters::operator=(const PinholeFullCamera::Parameters& other) {
    if (this != &other) {
...
std::ostream& operator<<(std::ostream& out, const PinholeFullCamera::Parameters& params) {
...
PinholeFullCamera::PinholeFullCamera() : m_inv_K11(1.0), m_inv_K13(0.0), m_inv_K22(1.0), m_inv_K23(0.0), m_noDistortion(true) {}

PinholeFullCamera::PinholeFullCamera(const std::string& cameraName, int imageWidth, int imageHeight, double k1, double k2, double k3, double k4, double k5,
                                     double k6, double p1, double p2, double fx, double fy, double cx, double cy)
...
PinholeFullCamera::PinholeFullCamera(const PinholeFullCamera::Parameters& params) : mParameters(params) {
    if ((mParameters.k1() == 0.0) && (mParameters.k2() == 0.0) && (mParameters.p1() == 0.0) && (mParameters.p2() == 0.0)) {
        m_noDistortion = true;
    } else {
        m_noDistortion = false;
...
void PinholeFullCamera::estimateIntrinsics(const cv::Size& boardSize, const std::vector<std::vector<cv::Point3f> >& objectPoints,
 ...
 */
void PinholeFullCamera::liftSphere(const Eigen::Vector2d& p, Eigen::Vector3d& P) const {
    liftProjective(p, P);
 ...
 */
void PinholeFullCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const {
...
void PinholeFullCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P, float image_scale) const {
 ...
 */
void PinholeFullCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p) const {
...
void PinholeFullCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p, float image_scalse) const {
 ...
 * \param p return value, contains the image point coordinates
 */
void PinholeFullCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p, Eigen::Matrix<double, 2, 3>& J) const {
    Eigen::Vector2d p_u, p_d;
 ...
 */
void PinholeFullCamera::undistToPlane(const Eigen::Vector2d& p_u, Eigen::Vector2d& p) const {
    Eigen::Vector2d p_d;
 ...
 */
void PinholeFullCamera::distortion(const Eigen::Vector2d& p_u, Eigen::Vector2d& d_u) const {
 ...
 * \return to obtain the distorted point: p_d = p_u + d_u
 */
void PinholeFullCamera::distortion(const Eigen::Vector2d& p_u, Eigen::Vector2d& d_u, Eigen::Matrix2d& J) const {
    // project 3D object point to the image plane
    double k1 = mParameters.k1();
    double k2 = mParameters.k2();
    double k3 = mParameters.k3();
...
void PinholeFullCamera::initUndistortMap(cv::Mat& map1, cv::Mat& map2, double fScale) const {
...
cv::Mat PinholeFullCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2, float fx, float fy, cv::Size imageSize, float cx, float cy,
                                                   cv::Mat rmat) const {
...
void PinholeFullCamera::writeParametersToYamlFile(const std::string& filename) const { mParameters.writeToYamlFile(filename); }
