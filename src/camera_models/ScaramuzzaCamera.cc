...
OCAMCamera::Parameters::Parameters() : Camera::Parameters(SCARAMUZZA), m_C(0.0), m_D(0.0), m_E(0.0), m_center_x(0.0), m_center_y(0.0) {
...
bool OCAMCamera::Parameters::readFromYamlFile(const std::string& filename) {
    ...
    for (int i = 0; i < SCARAMUZZA_POLY_SIZE; i++) m_poly[i] = static_cast<double>(n[std::string("p") + boost::lexical_cast<std::string>(i)]);

    n = fs["inv_poly_parameters"];
    for (int i = 0; i < SCARAMUZZA_INV_POLY_SIZE; i++) m_inv_poly[i] = static_cast<double>(n[std::string("p") + boost::lexical_cast<std::string>(i)]);
...
void OCAMCamera::Parameters::writeToYamlFile(const std::string& filename) const {
    ...
    fs << "poly_parameters";
    fs << "{";
    for (int i = 0; i < SCARAMUZZA_POLY_SIZE; i++) fs << std::string("p") + boost::lexical_cast<std::string>(i) << m_poly[i];
    fs << "}";

    fs << "inv_poly_parameters";
    fs << "{";
    for (int i = 0; i < SCARAMUZZA_INV_POLY_SIZE; i++) fs << std::string("p") + boost::lexical_cast<std::string>(i) << m_inv_poly[i];
    fs << "}";
...
std::ostream& operator<<(std::ostream& out, const OCAMCamera::Parameters& params) {
    ...
    for (int i = 0; i < SCARAMUZZA_POLY_SIZE; i++) out << std::string("p") + boost::lexical_cast<std::string>(i) << ": " << params.m_poly[i] << std::endl;
    ...
    out << "            ac " << params.m_C << std::endl << "            ad " << params.m_D << std::endl << "            ae " << params.m_E << std::endl;
    out << "            cx " << params.m_center_x << std::endl << "            cy " << params.m_center_y << std::endl;
...
OCAMCamera::OCAMCamera(const OCAMCamera::Parameters& params) : mParameters(params) { m_inv_scale = 1.0 / (params.C() - params.D() * params.E()); }
...
void OCAMCamera::estimateIntrinsics(const cv::Size& boardSize, const std::vector<std::vector<cv::Point3f> >& objectPoints,
...
void OCAMCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const {
    ...
    // xc_a = inv(A) * xc;
    Eigen::Vector2d xc_a(m_inv_scale * (xc[0] - mParameters.D() * xc[1]), m_inv_scale * (-mParameters.E() * xc[0] + mParameters.C() * xc[1]));
...
void OCAMCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p) const {
    ...
    p << xn[0] * mParameters.C() + xn[1] * mParameters.D() + mParameters.center_x(), xn[0] * mParameters.E() + xn[1] + mParameters.center_y();
}
...
cv::Mat OCAMCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2, float fx, float fy, cv::Size imageSize, float cx, float cy, cv::Mat rmat) const {
...
void OCAMCamera::readParameters(const std::vector<double>& parameterVec) {
    ...
    for (int i = 0; i < SCARAMUZZA_INV_POLY_SIZE; i++) params.inv_poly(i) = parameterVec.at(5 + SCARAMUZZA_POLY_SIZE + i);
...
void OCAMCamera::writeParameters(std::vector<double>& parameterVec) const {
    ...
    for (int i = 0; i < SCARAMUZZA_INV_POLY_SIZE; i++) parameterVec.at(5 + SCARAMUZZA_POLY_SIZE + i) = mParameters.inv_poly(i);
}
