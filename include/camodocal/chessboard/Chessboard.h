...
class Chessboard {
   public:
    Chessboard(cv::Size boardSize, cv::Mat& image, float squareSize, float markerSize);

    void findCorners(bool useOpenCV = false, bool useCharuco = true);
   ...
   private:
    bool findChessboardCorners(const cv::Mat& image, const cv::Size& patternSize, std::vector<cv::Point2f>& corners, std::vector<int>& ids, int flags,
                               bool useOpenCV, bool useCharuco);

    bool findChessboardCornersImproved(const cv::Mat& image, const cv::Size& patternSize, std::vector<cv::Point2f>& corners, int flags);
    ...
    int augmentBestRun(std::vector<ChessboardQuadPtr>& candidateQuads, int candidateDilation, std::vector<ChessboardQuadPtr>& existingQuads,
    ...
    void getQuadrangleHypotheses(const std::vector<std::vector<cv::Point> >& contours, std::vector<std::pair<float, int> >& quads, int classId) const;
